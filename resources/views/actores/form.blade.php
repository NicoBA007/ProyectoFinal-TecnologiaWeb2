        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-900 border border-gray-800 p-8 rounded-2xl shadow-2xl">
                ...
                <h2 class="text-2xl font-black text-white mb-6 uppercase tracking-tighter">
                    {{ isset($actor) ? 'Editar Actor' : 'Registrar Nuevo Actor' }}
                </h2>

                <form action="{{ isset($actor) ? route('actores.update', $actor->id_actor) : route('actores.store') }}"
                    method="POST">
                    @csrf
                    @if (isset($actor))
                        @method('PUT')
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="nombre_completo" value="{{ $actor->nombre_completo ?? '' }}"
                    ...
                    <div class="mb-8">
                        <label class="block text-gray-500 text-xs font-bold mb-2 uppercase">Nacionalidad</label>
                    ...
                    <div class="flex gap-4">
                        <button type="submit"
                            ...
                            {{ isset($actor) ? 'GUARDAR CAMBIOS' : 'AÑADIR AL ELENCO' }}
                        </button>
                        <a href="{{ route('actores.index') }}"
                            class="px-6 py-3 border border-gray-700 text-gray-400 rounded-xl hover:bg-gray-800 transition-all text-center">
                            CANCELAR
                        </a>
                    </div>
                </form>
