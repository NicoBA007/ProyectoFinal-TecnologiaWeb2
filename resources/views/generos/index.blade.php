<x-admin-layout>
        ...
        <div class="bg-gray-950 border border-white/5 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] overflow-hidden">
            <div class="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-gray-400 font-black uppercase tracking-[0.3em] text-[10px]">Clasificación por Género
                    </h3>
                </div>
    ...
    <script>
        ...
        async function cargarTabla() {
            const res = await fetch('{{ route('generos.index') }}', {
                headers: {
            ...
            json.data.forEach(g => {
                ...
                const btnEditar =
                    `<button onclick="abrirModal(${g.id_genero}, '${g.nombre}')" class="text-gray-400 hover:text-white font-black text-[9px] uppercase tracking-widest mr-4 transition-colors">Editar</button>`;
