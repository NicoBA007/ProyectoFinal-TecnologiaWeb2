<x-app-layout>
    {{-- Alpine.js y Contenedor Principal con pb-20 para espacio inferior --}}
    <div x-data="{ openModal: false }" class="min-h-screen bg-[#0f0f0f] text-white pb-20">

        {{-- Hero Section Dinámico --}}
        <div class="relative h-[450px] w-full overflow-hidden flex items-center bg-black">
            <div class="absolute inset-0 z-0">
                @php
                    $finalBg = $backdrop ?? ($peliculas->first() ? $peliculas->first()['poster_url'] : null);
                @endphp

                @if ($finalBg)
                    <img src="{{ $finalBg }}"
                        class="w-full h-full object-cover {{ $backdrop ? 'opacity-40' : 'opacity-20 blur-sm' }} transition-opacity duration-1000">
                @else
            ...
            <div class="relative z-10 max-w-7xl mx-auto px-8 w-full">
                <div class="flex items-center gap-4 mb-4">
                    <span
                        class="text-red-600 font-black tracking-[0.4em] uppercase text-[10px] block">Clasificación</span>
                    <button @click="openModal = true"
                        class="bg-white/10 hover:bg-red-600 backdrop-blur-md text-[9px] font-black px-4 py-1.5 rounded-full border border-white/10 transition-all uppercase tracking-widest flex items-center gap-2">
                        Cambiar Filtro
                ...
                <h1
                    class="text-7xl md:text-9xl font-black uppercase tracking-tighter italic leading-none drop-shadow-2xl">
                    {{ $clasificacion->codigo }}<span class="text-red-600">.</span>
                </h1>
                <p
                    class="text-gray-400 mt-6 max-w-2xl text-sm font-medium leading-relaxed italic border-l-4 border-red-600 pl-6">
                    {{ $clasificacion->nombre }} — {{ $clasificacion->descripcion }}
                </p>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-8 -mt-16 relative z-20">
            {{-- Toolbar --}}
            <div
                class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 border-b border-white/10 pb-8">
                <a href="{{ route('cartelera.index') }}"
                    ...
                    <span class="font-black text-[10px] uppercase tracking-[0.2em]">Volver a Cartelera</span>
                </a>

                <div class="relative">
                    <input type="text" id="movieSearch" placeholder="BUSCAR EN ESTA LISTA..."
                        class="bg-[#1a1a1a] border-none text-white text-[10px] font-black rounded-full pl-6 pr-12 py-3 w-72 focus:ring-1 focus:ring-red-600 transition-all uppercase placeholder:text-gray-600 outline-none">
                </div>
            </div>
            ...
            <div id="grid-peliculas"
                ...
                @forelse ($peliculas as $p)
                    <div
                        class="movie-card group relative bg-[#151515] rounded-2xl overflow-hidden transition-all duration-500 hover:-translate-y-3">
                ...
                @empty
                    <div class="col-span-full py-20 text-center">
                        <p class="text-zinc-600 font-black uppercase tracking-[0.3em] italic">No hay títulos disponibles
                        </p>
                    </div>
                @endforelse
            </div>
            ...
            <div class="mt-20 flex flex-col items-center gap-8">
                @if ($peliculas->total() > 0)
                    <div class="flex items-center gap-3">
                        ...
                        @if (!$peliculas->onFirstPage())
                            <a href="{{ $peliculas->previousPageUrl() }}"
                                class="group flex items-center justify-center w-12 h-12 bg-zinc-900 border border-white/10 rounded-xl transition-all hover:bg-red-600 hover:border-red-600">
                                <svg class="w-5 h-5 text-zinc-400 group-hover:text-white" fill="none"
                        ...
                        @else
                            <div
                                class="w-12 h-12 bg-zinc-900/30 border border-white/5 rounded-xl opacity-50 flex items-center justify-center">
                        ...
                        <div class="relative group min-w-[180px]">
                            ...
                            <div
                                class="relative px-8 py-3 bg-zinc-950 border border-white/10 rounded-2xl flex flex-col items-center">
                                <span class="text-[10px] font-black tracking-[0.2em] text-zinc-500 uppercase">
                    ...
                    <p class="text-zinc-600 text-[9px] font-black uppercase tracking-[0.3em] italic">
                        {{ number_format($peliculas->total()) }} Títulos filtrados
                    </p>
                @endif
            </div>
        </div>

        {{-- MODAL SELECTOR DE CLASIFICACIONES --}}
            ...
            <div
                ...
                <div class="flex justify-between items-center mb-8">
                    <h2 class="text-3xl font-black uppercase italic tracking-tighter">Explorar <span
                            class="text-red-600">Clasificaciones</span></h2>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($todasLasClasificaciones as $c)
                        <a href="{{ route('clasificacion.show', $c->id_clasificacion) }}"
                            ...
                            {{ $clasificacion->id_clasificacion == $c->id_clasificacion ? 'bg-red-600 ring-4 ring-red-600/20' : 'bg-[#222] hover:bg-zinc-800' }}">
                            ...
                            <div class="relative z-10 text-center px-4">
                                <span
                                    class="block text-2xl font-black uppercase italic tracking-tighter group-hover:scale-110 transition-transform duration-500">
                                    {{ $c->codigo }}
                                </span>
                                <span
                                    class="text-[10px] font-black opacity-60 uppercase tracking-widest group-hover:opacity-100 transition-opacity">
                                    {{ $c->nombre }}
                                </span>
                            </div>
