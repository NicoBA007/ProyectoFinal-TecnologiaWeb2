<x-admin-layout>
    <div class="py-10 max-w-5xl mx-auto sm:px-6 lg:px-8 text-white relative">
        ...
        {{-- Formulario de Registro de Países --}}
        <div class="mb-10">
            @include('paises.create')
        </div>

        {{-- TABLA DE REGISTRO GEOGRÁFICO --}}
        <div class="bg-gray-950 border border-white/5 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.6)] overflow-hidden">
            <div class="p-6 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-gray-400 font-black uppercase tracking-[0.3em] text-[10px]">Orígenes de Producción</h3>
                </div>
            </div>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead>
                        <tr class="text-gray-600 uppercase tracking-[0.2em] text-[9px] bg-white/[0.01]">
                            ...
                            <th class="py-4 px-6 border-b border-white/5">Nombre del País</th>
    ...
    <script>
        ...
        async function cargarTabla() {
            ...
            json.data.forEach(item => {
                ...
                const idFormateado = `#${String(item.id_pais).padStart(4, '0')}`;
                ...
                const btnEditar = `<button onclick="abrirModal(${item.id_pais}, '${item.nombre}')" class="text-gray-400 hover:text-white font-black text-[9px] uppercase tracking-widest mr-4 transition-colors">Editar</button>`;
                
                const btnEstado = item.activo
                    ? `<button onclick="cambiarEstado(${item.id_pais}, 'destroy', 'DELETE')" class="text-red-900 hover:text-red-500 font-black text-[9px] uppercase tracking-widest transition-colors">Desactivar</button>`
                    : `<button onclick="cambiarEstado(${item.id_pais}, 'reactivar', 'PATCH')" class="text-green-900 hover:text-green-500 font-black text-[9px] uppercase tracking-widest transition-colors">Reactivar</button>`;
                    ...
                    <tr class="hover:bg-white/[0.02] transition-all duration-300 ${!item.activo ? 'opacity-40' : ''}">
                        ...
                        <td class="py-4 px-6">
                            <div class="flex flex-col">
                                <span class="font-black text-white text-xs uppercase tracking-tight">${item.nombre}</span>
                            </div>
                        </td>
